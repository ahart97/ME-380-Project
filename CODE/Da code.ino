...
const int SERVOMIN = 200; // 'minimum' pulse length count (out of 4096) CHANGED TO 200
const int SERVOMAX = 450; // 'maximum' pulse length count (out of 4096) CHANGED TO 450
...
int home_val[6] = {SERVOMID, SERVOMID, SERVOMID, SERVOMID, SERVOMID, SERVOMID}; //Enter what values we want to start at
...
void loop() {
  if (Serial.available() > 0) {
    ...
    switch (valInput[0]) {
      ...
      //Don't see when we will ever need to use these
      /*case '<':
        for (i=0; i<6; i++) {
          val[i] = SERVOMIN;
        }
      ...
      case '-': //Note that moving them all only changes the pulse by +-5
        for (i=0; i<6; i++) {

          if(i%2 == 0){
            val[i] = max( val[i]-5, SERVOMIN ); //took a guess for which ones have to go up and which ones have to go down
          }
    ...
    Serial.print("Okay to update (y/n)?");
    update_ans = Serial.readString();

    if(update_ans == 'y') // added this so we can chnage multiple things at once
    {
      for (i=0; i<6; i++) 
      {
